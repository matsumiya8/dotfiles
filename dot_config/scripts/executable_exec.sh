...
proton() { 
    ...
    [ "${PROTON,,}" == "wine" ] && {
        WINEDLLOVERRIDES="${DEFAULTOVERRIDES}${DEFAULTOVERRIDES+:;}$GAMEOVERRIDES" WINEPREFIX="$PREFIXDIR$PREFIX" LANG="$LOCALE" wine "$FILE_PATH" $FILE_ARGS
        exit 0
    }
    [ -d "$COMPATDIR$PROTON" ] || {
    ...
    GAMEID=$PREFIX UMU_RUNTIME_UPDATE=$RUNTIMEUPDATE UMU_HTTP_TIMEOUT=1 PROTONFIXES_DISABLE=$NOFIXES PROTON_ENABLE_WAYLAND=$WAYLAND WINEDLLOVERRIDES="${DEFAULTOVERRIDES}${DEFAULTOVERRIDES+:;}$GAMEOVERRIDES" PROTON_USE_D7VK=$D7VK PROTONPATH="$COMPATDIR$PROTON" LANG="$LOCALE" PRESSURE_VESSEL_FILESYSTEMS_RW="$RWDIRS" umu-run "$FILE_PATH" $FILE_ARGS
    exit 0
}
